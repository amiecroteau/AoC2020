import itertools
raw_input = """15-16 m: mhmjmzrmmlmmmmmm
5-6 d: dcdddhzld
3-4 s: vqssdcbl
3-6 b: bbhbbbqbbb
10-11 q: cqqntqhqwwh
...
def convert_password_dict(line):
    password_dict = { }
    password_list = line.split()

    min_max = password_list[0].split("-")
    password_dict["min"]=int(min_max[0])
    password_dict["max"]=int(min_max[1])

    password_dict["char"] = password_list[1][0]
    password_dict["password"] = password_list[2]

    return password_dict
passwords_policies = [convert_password_dict(passwords_policies) for passwords_policies in split_input]


def meets_policy(pass_policy):
    char_count = pass_policy["password"].count(pass_policy["char"])
    return pass_policy["min"] <= char_count <= pass_policy["max"]

count_passwords = list(filter(meets_policy, passwords_policies))

total_passwords = len(count_passwords)


def secondPassword(pass_policy):
    first = pass_policy["min"]
    char_first = pass_policy["password"][first-1]==pass_policy["char"]

    second = pass_policy["max"]
    char_second = pass_policy["password"][second-1]==pass_policy["char"]

    return char_first ^ char_second

valid_passwords = list(filter(secondPassword, passwords_policies))
